...
fig = px.scatter(
    df_fil,
    x=(df_fil["Prog"] / df_fil["90s"]),
    y=(df_fil["Att"].iloc[:, 0] / df_fil["90s"]),
    title="Prog/90 vs Att/90s",
    hover_data=["Player", "Pos", "Comp", "Nation"],
    color="Player",
    size="90s",
    text="Player",
    labels={"x": "Prog/90", "y": "Att/90s"},
)
